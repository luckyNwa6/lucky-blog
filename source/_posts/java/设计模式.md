...
cover: https://imgs.luckynwa.top/profile/blog/designIcon.png
categories: 后端
tags: Java
comments: true
abbrlink: 45939
summary: >-
  这里是码农观测站，这篇文章介绍了设计模式的核心概念、分类及六大原则，涵盖23种经典设计模式，分为创建型、结构型和行为型三大类，包括工厂方法、抽象工厂、单例等5种创建型模式，外观、适配器、代理等7种结构型模式，以及模板方法、观察者、策略等11种行为型模式，同时详细阐述了面向对象设计的六大原则：单一职责原则要求类只负责一项职责，里氏替换原则强调子类不能改变父类原有功能，依赖倒置原则主张面向接口编程，接口隔离原则提倡建立单一接口以降低耦合，迪米特原则即最少知道原则以减少类间交互，开闭原则则要求对扩展开放、对修改关闭，此外还通过示例讲解了简单工厂模式和工厂方法模式的实现方式、适用场景及优缺点，如简单工厂模式通过一个工厂类根据参数创建不同实例但违背开闭原则，而工厂方法模式通过抽象工厂和具体工厂解耦，使系统更易扩展和维护。
