...
abbrlink: 29614
summary: >-
  启动小维AI摘要模块⚡……运算完成！这篇文章主要介绍了MySQL数据库的核心知识，包括数据库设计时的字符存储规则，如一个汉字占3个字节，varchar(255)最多存85个汉字，以及数据类型详解，涵盖整型int及其不同长度变体、char与varchar的区别及适用场景、text类型用于存储大量文本、日期时间类型如datetime、date、time和timestamp的特性，还讲解了基本的数据库操作如增删改查（CRUD）语法，多表查询注意事项，MySQL不区分大小写的特性，排序方式asc和desc，HAVING分组后条件，数据库设计的六大范式概念，重点阐述了第一范式要求属性原子性、第二范式消除非主属性对码的部分函数依赖、第三范式消除非主属性对码的传递函数依赖，并通过实例说明如何通过拆表满足更高范式要求，整体内容围绕MySQL数据库的基础设计、数据类型选择、操作方法和规范化设计展开。
