...
abbrlink: 53014
summary: >-
  启动小维AI摘要模块⚡……运算完成！这篇文章主要介绍了网络通信中Token（令牌）的作用及其安全机制，Token是用于验证用户身份和授权访问的一串随机字符串，服务器通过它识别用户身份和权限，同时可利用加密和签名保障信息安全，但若Token没有合理过期时间则存在被劫持风险，因此建议将accessToken的过期时间设置得较短以提高安全性，但这样会导致用户频繁重新登录，影响体验，为此引入双token机制，即accessToken和refreshToken，前者用于频繁请求且过期时间短，后者用于获取新的accessToken且不易被频繁使用，从而在保证安全的同时减少用户重新登录的频率，文章还提到可在响应拦截器中处理token过期问题，当后端返回401状态码时调用刷新token接口或跳转登录页，并简述了基于oauth2的单点登录及鉴权相关内容。
