...
# cover: https://imgs.luckynwa.top/profile/blog/mavenIcon.png
categories: 后端
tags: Java
comments: true
abbrlink: 47280
summary: >-
  启动小维AI摘要模块⚡……运算完成！这篇文章主要介绍了开发中常用的依赖和工具类，包括Lombok注解库的使用方法及其在IDEA中的安装配置，通过@Data、@NoArgsConstructor和@AllArgsConstructor等注解简化实体类的getter/setter、构造函数编写，还提到通过properties标签规范管理配置，同时讲解了SpringBoot单元测试的依赖配置及测试类编写方式，涉及@SpringBootTest和@SpringRunner等注解的使用，另外介绍了Fastjson2作为JSON处理工具的优势及前后端交互示例代码，包括后端接收JSONObject参数处理，前端采用post请求传递数据，还描述了WebMvcConfigurer接口实现类InterceptorConfig配置拦截器路径匹配规则及静态资源处理的方法，最后展示了SpringBoot启动文本的基础结构，整体内容围绕提升开发效率的工具配置及基础用法展开。
