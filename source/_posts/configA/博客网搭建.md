...
cover: https://imgs.luckynwa.top/profile/blog/hexoIcon.png
categories: 网站搭建
tags: Hexo
comments: true
abbrlink: 17956
summary: >-
  启动小维AI摘要模块⚡……运算完成！这篇文章详细介绍了如何使用Hexo框架配合butterfly主题快速搭建一个美观的博客网站，首先讲解了环境准备包括安装node、git和vscode，并提供了相关配置链接，接着指导在桌面创建blog文件夹并通过命令行初始化Hexo项目，生成本地页面并通过localhost:4000访问，随后介绍了将博客部署到GitHub和Vercel的方法，包括仓库创建、配置修改及自动化部署工具安装，还重点讲解了butterfly主题的安装与配置，涉及主题下载、配置文件修改、渲染器安装、URL优化、本地搜索功能添加及文章和页面创建指令，此外还提及了在VS
  Code中项目启动问题的解决方法，以及集成twikoo评论系统的步骤，包括docker部署、nginx反向代理配置和邮箱通知设置，整体内容全面且实操性强，适合初学者按步骤搭建个人博客。
