...
abbrlink: 17956
summary: >-
  这里是码农观测站，这篇文章详细介绍了如何使用Hexo框架快速搭建个人博客网站，并配合butterfly主题实现美观的页面设计，首先需要安装node、git和vscode环境，接着在桌面新建blog文件夹并通过命令行初始化Hexo项目，生成本地页面后可通过localhost:4000访问，部署方面支持GitHub和Vercel两种方式，其中GitHub需创建特定格式的仓库并修改配置文件，Vercel则通过GitHub登录直接导入项目，还提供了butterfly主题的安装步骤，包括克隆主题、修改配置文件、安装必要渲染器和插件，同时介绍了文章创建、本地搜索、URL优化等功能，此外还解决了VS
  Code启动权限问题及twikoo评论系统的部署方法，包括Docker部署、域名配置和nginx反向代理设置，整体流程清晰，适合新手参考。
