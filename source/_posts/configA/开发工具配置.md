---
title: 开发工具
description: 磨刀不误砍柴工，工欲善其事
cover: https://imgs.luckynwa.top/openApi/lucky/yys/321
categories: 工具
tags: Tools
comments: true
abbrlink: 1839
summary: >-
  启动小维AI摘要模块⚡……运算完成！这篇文章详细介绍了IntelliJ IDEA
  2023.2版本的完整安装配置与优化流程，包括软件卸载时需勾选两个删除选项、安装时需将软件放在D:\soft\idea2023.2目录并复制jetbra文件夹内容，通过运行install-all-users.vbs脚本和配置idea.vmoptions文件完成激活，使用提供的激活码激活软件，同时讲解了IDEA的系列优化设置，如保存时自动格式化代码和优化导入、开启自动编译与热部署（需引入spring-boot-devtools依赖）、配置类头文件自动生成模板、调整滚轮缩放字体、设置自动导包与删除无用包、显示行号和方法分隔符、启用鼠标悬浮提示与取消大小写敏感提示、配置编码避免乱码、取消tab页单行显示改为多行、自定义字体大小与行间距，还推荐了Lombok、MybatisX、Rainbow
  Brackets等实用插件及其配置技巧，旨在帮助用户高效搭建个性化且功能强大的IDEA开发环境。
