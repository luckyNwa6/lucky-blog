...
cover: https://imgs.luckynwa.top/openApi/lucky/yys/241
categories: 前端
tags: LayUi
comments: true
abbrlink: 36841
summary: >-
  启动小维AI摘要模块⚡……运算完成！这篇文章主要记录了LayUi在使用过程中遇到的两个典型问题及其解决方案，首先是低分辨率下操作栏按钮点击无效的bug，该问题是由于屏幕分辨率过低导致操作按钮被折叠成省略号弹出框，而弹出框不在表格监听范围内导致点击失效，解决方法是通过给按钮添加data-row属性传递整行数据为全局变量，再在外部通过全局变量获取点击数据并处理相应逻辑，同时详细展示了HTML模板、JS模板及事件绑定方法的代码实现；其次是LayUi全选功能实现包含全部页面数据的思路与方法，其核心是通过后端返回的ids与表格渲染时的每行id进行比较来设置选中状态，并在表格重载时处理好相关全局变量如ids、lists和tableIds的初始化和使用，文章提供了详细的代码片段和实现步骤，为LayUi使用者提供了实用的bug解决和功能实现参考。
