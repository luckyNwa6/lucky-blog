...
<style>
    .muyu-layout {
        background: linear-gradient(180deg, #1a1a1a 0%, #2d1b00 100%);
    ...
    #muyu-box img:active {
        transform: scale(0.9);
        filter: drop-shadow(0 0 15px rgba(255, 215, 0, 0.8)) drop-shadow(0 0 30px rgba(255, 215, 0, 0.6));
    }
    @keyframes golden-glow {
        0% { filter: drop-shadow(0 0 5px rgba(255, 215, 0, 0.5)); }
        50% { filter: drop-shadow(0 0 20px rgba(255, 215, 0, 0.8)) drop-shadow(0 0 40px rgba(255, 215, 0, 0.6)); }
        100% { filter: drop-shadow(0 0 5px rgba(255, 215, 0, 0.5)); }
    }
    .golden-flash {
        animation: golden-glow 0.5s ease;
    }
     #jishu-box{
        top: 55px;
        position: absolute;
    ...
    <script>
        ...
        muyu.addEventListener("click", function () {
            ...
            // 金光闪闪效果
            muyu.classList.add('golden-flash');
            setTimeout(() => { muyu.classList.remove('golden-flash'); }, 500);
        });
    </script>
